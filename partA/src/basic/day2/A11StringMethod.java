...
public class A11StringMethod {
    ...
    public static void main(String[] args) {
        ...
        // 그리고 괄호 안의 값들은 메소드 처리에 필요한 입력 데이터 -> 메소드의 `인자`라고 합니다. 
        //        메소드가 처리한 결과는 출력 데이터 -> 메소드의 `리턴(반환)값`이라고 합니다.

        // 자바 String의 중요한 메소드 7가지씩 조사에서 테스트 해 보기
        // 메소드 인자의 개수와 형식에 따라 분류해 보기

        String message = "Hello, World!";

        // 1. 메소드 괄호 안에 아무것도 없는 것: length, isEmpty, toUpperCase, toLowerCase
        System.out.println("message.length(): " + message.length());
        // ㄴ> int(출력 리턴) java.lang.String(소속패키지).length(이름)()(입력 인자X)
        ...
        // 현재 message에서 마지막 인덱스는 얼마인가 하면...... length-1 (0부터 시작)
        ...
        // 문자열 안에서 원하는 단어의 위치를 찾기 -> 시작 위치 index 값을 리턴, 단어가 없으면 -1을 리턴
        ...
        // 4. 메소드 괄호 안에 정수 2개를 쓰는 것: substring (문자열 부분 추출: 시작 인덱스, 마지막 인덱스 값을 인자로 추가)
        ...
        // 추출할 때 마지막 인덱스 4는 포함 안 함. 결국 추출하는 문자 개수는 endIndex-beginIndex 공식이 성립합니다.

        message = "hello";
        System.out.println("message.equls(\"Hello\"): " + message.equals("Hello"));
        System.out.println("message.equls(\"hello\"): " + message.equals("hello"));
        ...
        // 질문에 공유
        // 1) 메소드의 리턴 타입에 따라 결과를 저장할 변수를 일치하는 형식으로 선언하기
        String temp = message.toUpperCase().substring(3, 5); 
        //                      ㄴ 2) 문자열 리턴 값으로 또 메소드 실행할 수 있음
        System.out.println("message.toUpeerCase().substring(3, 5): " + message.toUpperCase().substring(3, 5));
        ...
        //       단,    message.charAt(1)를 예시로 하면 message는 메소드의 실행 주체.
        // 그리고 리턴이 없는 메소드가 있을 수 있으며, 그때는 void로 표현됩니다.
