...
/*문제: 어떤 학생의 중간고사 총점은 322, 과목수는 4개 입니다. 모두 항상 정수입니다.
	이 학생의 평균을 구하려고 합니다. 변수는 총점은 sum, 과목개수는 count로 합니다.
	평균(나눗셈 연산은 / )를 계산합니다. 
	* 정수 / 정수 = 결과가 정수(몫), * 실수/정수, 정수/실수, 실수/실수 = 결과가 실수
	1) 평균 저장 변수를 정수형 avgInt  2) 평균을 저장할 변수를 실수형 avgDouble 각각 해보세요.
...
public class A04DataCasting {
	...
	public static void main(String[] args) {
		...
		sum =322;
		count=4;
		avgInt = sum/count;			//정수 결과값? 80
		avgDouble = sum/count;		//1) 80.0
		...
		avgDouble = (double)sum/count;			//2)sum을 강제로 변환(캐스팅)	- 1)과의 차이점.
		// 322.0 나누기 4 => 실수값 소수점 이하 자리수 계산
		System.out.println("실수형식으로 변환 후 평균 :" + avgDouble);

		System.out.println("형변환(캐스팅)에는 자동과 강제 2가지 캐스팅이 있습니다.");
...
/*
 * 복습: 기본 자료형의 크기 순서대로 나열해 봅시다
 *  
 * byte < short < int < long < float < double
 *  1      2       4     8       4       8		바이트
 * 
 * char 문자형을 코드값을 구하기 위해 정수로 캐스팅할 때는 0~65535를 충분히 저장할 수 있는 
 * int 정수형으로 캐스팅합니다.
 */